...
public class MainActivity extends Activity {
    ...
    private void extractOffers(int expectedPage) {
        ...
        String js = "javascript:(()=>{" +
            ...
            "document.querySelectorAll('a[href*=\\\"offerId=\\\"]').forEach(a=>{let href=a.href;if(new URL(href).pathname!==expected||seen.has(href))return;let c=a.closest('article');if(!c)return;let p=price(c.textContent),text=n(c.textContent);if(p==null)return;seen.add(href);out.push({price:p});if(/(^|[^a-z0-9_])lukiwa([^a-z0-9_]|$)/i.test(text))own=p});" +
